# code adapted from transformers library:
...
class FlaxDataCollatorForT5MLM:
    ...
    def __call__(self, batch) -> Dict[str, np.ndarray]:
        ...
        # batch = {
        #     k: torch.tensor([examples[i][k] for i in range(len(examples))])
        ...
        mask_indices = np.array([self.random_spans_noise_mask(expandend_input_length) for i in range(batch_size)])
        ...
        # print(f"input_ids_sentinel.shape: {input_ids_sentinel.shape}")
        # print(f"labels_sentinel.shape: {labels_sentinel.shape}")
...
def group_texts(examples, expanded_inputs_length):
    ...
    result = [
        concatenated_examples[i : i + expanded_inputs_length]
