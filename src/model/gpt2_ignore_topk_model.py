...
class GPT2IgnoreTopK(GPT2forGeneration):
    """
    GPT-2 model with ignore-topk pruning functionality.
    
    This implementation follows the same pruning approach as T5IgnoreTopK,
    but adapted for the GPT-2 architecture. The pruning method keeps track
    of weight changes during training and selectively prunes the largest weight
    ...
    def snapshot_base_weights(self):
        """Take a snapshot of model weights to use as reference for pruning"""
        self.theta_base = {}
    ...
    def find_kth_value(self, all_deltas, k_fraction, tensor_name=None):
        """
        Find the k-th largest value across all deltas without concatenating into one tensor.
        Uses binary search for large tensors to avoid memory issues.
        """
    ...
    def apply_ignore_topk_pruning(self):
        """Apply the ignore-topk pruning method to the model weights"""
    ...
    def init_for_training(self):
        """Initialize for training - take a snapshot before training starts"""
    ...
    def forward(self, batch):
        """
        Forward pass with pruning control logic.
        
        This is effectively the same as T5IgnoreTopK but uses the GPT2 forward pass.
        It counts steps during training and applies pruning when tau steps are reached.
        """
